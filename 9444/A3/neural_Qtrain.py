...
NUM_EPISODES = 100+100#TEST_FREQUENCY  # Episode limitation
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    eval_n = tf.layers.dense(state_in, 64, tf.nn.relu, kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer)
    q_values = tf.layers.dense(eval_n, action_dim, kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer)
    ...
    for episode in range(num_episodes):
        ...
        for step in range(ep_max_steps):
            ...
            if done:
                break
        if test_mode:
            test_ave_reward += ep_reward
            count_test += 1
        #print(test_ave_reward)
        total_reward += ep_reward
        test_or_train = "test " if test_mode else "train"
        print("end {0} episode {1}, ep reward: {2:5.1f}, ave reward: {3:10.4f}, test_ave_reward: {4:10.4f}, \
            Batch presentations: {5}, epsilon: {6}".format(
            test_or_train, episode, ep_reward, total_reward / (episode + 1), test_ave_reward/(count_test+1),
