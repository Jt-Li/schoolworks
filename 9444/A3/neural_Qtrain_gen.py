...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon, iscontinuous, action_spaces
    replay_buffer = []
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    w1 = tf.get_variable('w1', shape=[state_dim, hidden_nodes], initializer = w_init)
    b1 = tf.get_variable('b1', shape=[1, hidden_nodes], initializer = b_init)
    layer_1 = tf.nn.relu(tf.matmul(state_in, w1) + b1)

    w2 = tf.Variable(tf.random_normal(shape=[hidden_nodes, hidden_nodes]))
    b2 = tf.Variable(tf.random_normal(shape=[1, hidden_nodes]))

    layer_2 = tf.nn.relu(tf.matmul(layer_1, w2) + b2)

    w3 = tf.get_variable('w3', shape=[hidden_nodes, action_dim], initializer = w_init)
    b3 = tf.get_variable('b3', shape=[1, action_dim], initializer = b_init)

    q_values = tf.matmul(layer_2, w3) + b3
...
def get_env_action(action):
    ...
    if iscontinuous:
        float_action = (action- (action_spaces - 1)/ 2) / ((action_spaces - 1) / 4)
        action = [float_action]
...
def qtrain(env, state_dim, action_dim,
    ...
    for episode in range(num_episodes):
        ...
        print("end {0} episode {1}, ep reward: {2:5.1f}, ave reward: {3:10.7f}, \
